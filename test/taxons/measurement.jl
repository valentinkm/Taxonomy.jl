...
@testset "Fixed Struct Tests" begin
    ...
    @test typeof(Taxonomy.Judgements.strip_fixed(fixed_value)) == Float64

    # Test the strip_fixed function with Fixed object
    @test Taxonomy.Judgements.strip_fixed(fixed_value) == 0.7

    # Test the strip_fixed function with a regular number
    regular_number = 2.5
    @test Taxonomy.Judgements.strip_fixed(regular_number) == 2.5

    test_factor_1 = Measurement(
        n_variables=2,
        loadings=[Fixed(1), 0.4],
        factor_variance=Fixed(1)
    )
